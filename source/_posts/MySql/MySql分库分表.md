...
### mycat
1. 高可用：zookeeper，keepalive,HAProxy
2. 负载均衡
3. sql解析路由
...
### mysql主从复制细节
从库状态IO Thread和SQL Thread都为YES才能正常启动;
...
### 主从复制延迟的点
当SQL Thread将relay_log中的数据读取出来同步到从机的数据文件中去,这里发生的是随机IO,需要先查找具体更新数据的内存地址,然后再进行修改,这个过程可能比较耗时;且整个过程是单线程执行的;
