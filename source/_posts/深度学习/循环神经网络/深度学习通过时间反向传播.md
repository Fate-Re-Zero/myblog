...
输入表示为$x_t$，输出表示为$o_t$。
回想一下我们在 subsec_rnn_w_hidden_states中的讨论，
...
（图：比较RNN中计算梯度的策略，3行自上而下分别为：随机截断、常规截断、完整计算）

 fig_truncated_bptt说明了
当基于循环神经网络使用通过时间反向传播
...
（图：上图表示具有三个时间步的循环神经网络模型依赖关系的计算图。未着色的方框表示变量，着色的方框表示参数，圆表示运算符）

正如刚才所说， fig_rnn_bptt中的模型参数是
