...
假设网络一次只处理具有$n$个时间步的子序列。
 fig_timemachine_5gram画出了
从原始文本序列获得子序列的所有不同的方式，
其中$n=5$，并且每个时间步的词元对应于一个字符。
请注意，因为我们可以选择任意偏移量来指示初始位置，所以我们有相当大的自由度。

（图：分割文本时，不同的偏移量会导致不同的子序列）

因此，我们应该从 fig_timemachine_5gram中选择哪一个呢？
事实上，他们都一样的好。
然而，如果我们只选择一个偏移量，
