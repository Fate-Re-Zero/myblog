...
这听起来有点抽象，那我们来看一下 fig_pacman里的例子。
第一组存在不包含在集合内部的线段，所以该集合是非凸的，而另外两组则没有这样的问题。

（图：第一组是非凸的，另外两组是凸的。）

接下来来看一下交集 fig_convex_intersect。
...
在 fig_nonconvex中连接$a$和$b$的线段需要包含一部分既不在$\mathcal{X}$也不在$\mathcal{Y}$中。
...
事实上，我们一直在使用这个技巧。
比如权重衰减 sec_weight_decay，在目标函数中加入$\frac{\lambda}{2} |\mathbf{w}|^2$，以确保$\mathbf{w}$不会增长太大。
...
同样，我们之前也遇到过，例如在 sec_rnn_scratch中处理梯度截断时，我们通过
...
（图：Convex Projections.）

投影的数学定义听起来可能有点抽象，为了解释得更清楚一些，请看 fig_projections。
图中有两个凸集，一个圆和一个菱形。
...
这是 fig_projections中菱形例子的一个广义版本。
