...
正如在 动量法中那样，我们可以根据其特征分解$\mathbf{Q} = \mathbf{U}^\top \boldsymbol{\Lambda} \mathbf{U}$重写这个问题，来得到一个简化得多的问题，使每个坐标都可以单独解出：
...
与 小批量随机梯度下降一节中的实验相比，这里使用更大的学习率来训练模型。
...
* 在深度学习问题上，AdaGrad算法有时在降低学习率方面可能过于剧烈。我们将在 Adam算法一节讨论缓解这种情况的策略。
