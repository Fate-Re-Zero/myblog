...
首先，我们读取 循环神经网络的从零开始实现中使用的时间机器数据集：
...
与 循环神经网络的从零开始实现中定义的`init_rnn_state`函数一样，
