...
（图：使用循环神经网络编码器和循环神经网络解码器的序列到序列学习）
...
如 fig_seq2seq所示，可以使用循环神经网络来设计编码器。
...
循环层返回变量的说明可以参考 sec_rnn-concise。
...
总之，上述循环神经网络“编码器－解码器”模型中的各层如
 fig_seq2seq_details所示。

（图：循环神经网络编码器-解码器模型中的层）
...
与训练类似，序列开始词元（“&lt;bos&gt;”）
在初始时间步被输入到解码器中。
该预测过程如 fig_seq2seq_predict所示，
当输出序列的预测遇到序列结束词元（“&lt;eos&gt;”）时，预测就结束了。

（图：使用循环神经网络编码器-解码器逐词元地预测输出序列。）

我们将在 sec_beam-search中介绍不同的序列生成策略。
