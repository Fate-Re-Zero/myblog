...
这些边界框被称为*锚框*（anchor box）我们将在 单发多框检测（SSD）中设计一个基于锚框的目标检测模型。
...
那么锚框的宽度和高度分别是$hs\sqrt{r}$和$hs/\sqrt{r}$。
...
对于两个边界框，它们的杰卡德系数通常称为*交并比*（intersection over union，IoU），即两个边界框相交面积与相并面积之比，如 fig_iou所示。
交并比的取值范围在0和1之间：0表示两个边界框无重合像素，1表示两个边界框完全重合。

（图：交并比是两个边界框相交面积与相并面积之比。）
