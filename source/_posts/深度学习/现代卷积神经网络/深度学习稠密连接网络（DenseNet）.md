...
（图：ResNet（左）与 DenseNet（右）在跨层连接上的主要区别：使用相加和使用连结。）

如 fig_densenet_block所示，ResNet和DenseNet的关键区别在于，DenseNet输出是*连接*（用图中的$[,]$表示）而不是如ResNet的简单相加。
...
DenseNet使用了ResNet改良版的“批量规范化、激活和卷积”架构（参见 残差网络（ResNet）中的练习）。
我们首先实现一下这个架构。
...
这里我们设成4，从而与 残差网络（ResNet）的ResNet-18保持一致。
