...
有趣的是，在网络的最底层，模型学习到了一些类似于传统滤波器的特征抽取器。 fig_filters是从AlexNet论文 复制的，描述了底层图像特征。

（图：AlexNet第一层学习到的特征抽取器。）
...
AlexNet和LeNet的架构非常相似，如 fig_alexnet所示。
...
AlexNet通过暂退法（ sec_dropout）控制全连接层的模型复杂度，而LeNet只使用了权重衰减。
为了进一步扩充数据，AlexNet在训练时增加了大量的图像增强数据，如翻转、裁切和变色。
这使得模型更健壮，更大的样本量有效地减少了过拟合。
在 sec_image_augmentation中更详细地讨论数据扩增。
...
现在AlexNet可以开始被训练了。与 卷积神经网络（LeNet）中的LeNet相比，这里的主要变化是使用更小的学习速率训练，这是因为网络更深更广、图像分辨率更高，训练卷积神经网络就更昂贵。
