...
简单性以较小维度的形式展现，
我们在 模型选择、欠拟合和过拟合 讨论线性模型的单项式函数时探讨了这一点。
此外，正如我们在 权重衰减 中讨论权重衰减（$L_2$正则化）时看到的那样，
参数的范数也代表了一种有用的简单性度量。
...
回想一下 多层感知机网络图中带有1个隐藏层和5个隐藏单元的多层感知机。
当我们将暂退法应用到隐藏层，以$p$的概率将隐藏单元置为零时，
结果可以看作一个只包含原始神经元子集的网络。
比如在 fig_dropout2中，删除了$h_2$和$h_5$，
...
同样，我们使用 图像分类数据集中引入的Fashion-MNIST数据集。
