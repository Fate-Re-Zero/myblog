...
如果我们想参加Kaggle比赛，首先需要注册一个账户（见 fig_kaggle）。

（图：Kaggle网站）
:width:`400px`

在房价预测比赛页面（如 fig_house_pricing 所示）的"Data"选项卡下可以找到数据集。我们可以通过下面的网址提交预测，并查看排名：
...
我们只能在官方测试集中评估我们的模型。
在 fig_house_pricing 中，"Data"选项卡有下载数据的链接。

开始之前，我们将使用`pandas`读入并处理数据，
这是我们在 sec_pandas中引入的。
...
首先，我们将所有缺失的值替换为相应特征的平均值。然后，为了将所有特征放在一个共同的尺度上，
...
（请参见 subsec_classification-problem）。
